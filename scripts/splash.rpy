...
image intro:
    ...
    0.5
    "mod_assets/letter.png" with Dissolve(0.5, alpha=True)
    2.5
    "white" with Dissolve(0.5, alpha=True)
    0.5
...
label splashscreen:
    ...
    $ pause(3.0+3.0 - (datetime.datetime.now() - starttime).total_seconds())
    hide intro with Dissolve(max(0, 3.5+3.0 - (datetime.datetime.now() - starttime).total_seconds()), alpha=True)
    if persistent.playthrough == 2 and renpy.random.randint(0, 3) == 0:
        $ splash_message = renpy.random.choice(splash_messages)
    show splash_warning "[splash_message]" with Dissolve(max(0, 4.0+3.0 - (datetime.datetime.now() - starttime).total_seconds()), alpha=True)
    $ pause(6.0+3.0 - (datetime.datetime.now() - starttime).total_seconds())
    hide splash_warning with Dissolve(max(0, 6.5+3.0 - (datetime.datetime.now() - starttime).total_seconds()), alpha=True)
    $ pause(6.5+3.0 - (datetime.datetime.now() - starttime).total_seconds())
    $ config.allow_skipping = True
    return
